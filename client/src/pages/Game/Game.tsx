...
export default function Game() {
	...
	const mainComponent = () => {
		if (state === 0) return <GameRoom /> // 대기실
		if (state === 1) {
			if (turn === 0) return <GameTeller imteller={imteller} nickname={nickname} /> // 텔러의 그림 선택 or 비슷한 그림 선택
			if (turn === 1) return <GameChoice /> // 텔러그림 맞추기
